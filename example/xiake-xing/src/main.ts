 ...
 * 侠客行 终端 REPL（M0）
 *
 * 输入处理采用「队列 + 串行排水」模式（与 tide-cellar / mini-rpg 同款——
 * 这份样板每包抄一遍，已记入 docs/engine-feedback.md F1，待抽进工具集）：
...
async function main() {
  ...
  console.log('=== 侠客行 · xiake-xing（MUD 文字游戏引擎 v' + ENGINE_VERSION + '） ===');
  console.log('终南山下，青石镇里。输入 help 查看命令，输入 quit 退出。');
  console.log('');
