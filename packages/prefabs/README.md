...
`defineRoom` 世界给玩家预挂 `Backtrack`（来路栈，进快照）后，`回/退/back`
命令**原路折返**：弹栈消费——退掉当前房记录、回到上一间，连按可逐站走回
出生点。守卫照走（有拦的房间后退照被拦）；回退移动本身不再入栈（否则
back 会在最后两间打转）。栈有界（32）。

## 进房信息呈现（0.14，对照 xkx）

**房间块三档**：首次/详细 = 完整块；重复 = 短描述档。描述全角两格缩进。
**出口信息**：略图**未开**时显示出口行（死路明说"这里没有任何出口"）；
略图开着时平面方向由小图承担，文本行让位——up/down 不入图，恒以
「另有出口：上、下。」补充行交代。

**长/短双描述**：`defineRoom({ short: '一行氛围。' })`——重复进房（自动简略）
显示【名】+ short + 出口，不再光秃秃只报名；不写 `short` 回退旧行为。
...
- 活体逐行：会动的（Position）+ 钉在房间的 NPC（Located 且带对话/任务/生命），
  主名可点（tag:entity），别名与重名计数跟在名字后；
- **姿态短语**：内容层给 NPC/怪挂 `Pose`（一句话，不带句尾标点）即拼在名字后；
- 地上物全列：场景摆设也可见（能 `look` 到的东西不在列示里隐身），
  拿不动的标注「（拿不动）」。

**环境行预留位**：房间块的描述与出口之间为时辰/天气留了位置（当前无钩子代码，
落地时插一行即可，房间块结构不再动）。
