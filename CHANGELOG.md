...
主题：**新内容包开工 + 首批真实游玩反馈的体验修复**——侠客行（武侠题材
文字 RPG）M0 骨架落地后实机试玩，浏览器端撞出的渲染器缺陷与移动/地图
信息缺口一并修掉。引擎（`@mud/ecs-engine`）零变更。
...
### prefabs：移动与地图的信息补全（@mud/prefabs 0.11.0）
...
- **地图自解释**：`map` / `worldmap` 末尾补「地点：A（此） · B」 /
  「区域：…（此）」清单——ASCII 图看不懂符号时，地名一目了然
- VerboseCommand 开关判定改为组件存在性（原 before/after 对比会把
  「事件系统没注册」误报成「没有详略开关」——侠客行浏览器实测撞出）
- prefabs 136 例（+详略/出口）；mini-rpg / tide-cellar / xiake-xing /
  docs 验证示例的地图断言同步补清单行
