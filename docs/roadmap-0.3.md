...
## v0.5.0 窗口（开发中，2026-09-02 起）

两条主线并行推进，一次发版。**含 breaking**（0.4.x 未 publish）。

### 主线 1：会"活"的世界（内容表达力第二步）★ 战斗/巡逻/死亡核心已完成
...
- **`ctx.spawn` 注入**（SystemContext 已实现；CommandContext 按铁律不加）——补上"系统内
