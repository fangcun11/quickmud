...
`worldmap` 画区域之间的连接，迷雾由 `Visited` 组件决定：未探明的方向只画一小段
**断线**——告诉你这边有路，但不剧通向哪里。房间块是 xkx 式的：**出口恒显**
（死路也明说）、描述全角缩进、**人和物分开列**——NPC 一行一体、名字带别名
（「狼」「wolf」都能敲，两只同名怪用 `野狼2` 指定第二只，网页端实体名可点击
查看）、地上摆设可见并标注拿不动；伤重了流内直接警示，不用翻状态。重复进房
走**短描述档**（`defineRoom({ short })`），一行氛围不啰嗦。
...
地窖(你)      ← 三层剖面亮两层：钟楼未探明留白；跨层（up/down）不画连线
```
